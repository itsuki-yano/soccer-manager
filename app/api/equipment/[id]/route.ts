...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const rows = await getSheetData("equipment!A:G");
    const idx = rows.findIndex((r) => r[0] === id);
    if (idx < 0) return NextResponse.json({ error: "not found" }, { status: 404 });
    await updateRow("equipment", idx + 1, [id, body.name, body.quantity, body.memo, body.parentId, body.order, body.imageUrl ?? ""]);
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    for (let i = rows.length - 1; i >= 1; i--) {
      if (rows[i][0] === id || childIds.includes(rows[i][0])) toDelete.push(i + 1);
    }
